...
void vUlpInit()
{
   ...
   else
   {
      ...
      //
      if ( (RCC->CFGR & RCC_CFGR_MCOSEL_HSI_Msk) == RCC_CFGR_MCOSEL_HSI )
      {
         SET_BIT(RCC->CFGR, RCC_CFGR_STOPWUCK);
      }
      else
      {
         ...
         //
         configASSERT( (RCC->CFGR & RCC_CFGR_MCOSEL_MSI_Msk) == RCC_CFGR_MCOSEL_MSI );
      }
   }
}
...
void vUlpPreSleepProcessing()
{
   ...
   {
      useDeepSleep = pdTRUE;
      MODIFY_REG(PWR->CR, PWR_CR_LPSDSR_Msk, 1);
   }
   else if ((xDeepSleepForbiddenFlags & ~ulpPERIPHERALS_OK_IN_STOP1) == 0)
   {
      useDeepSleep = pdTRUE;
      MODIFY_REG(PWR->CR, PWR_CR_LPSDSR_Msk, 0);
   }
